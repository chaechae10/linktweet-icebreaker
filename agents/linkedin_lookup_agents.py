...
def lookup(name: str) -> str:
    ...
    tools_for_agent = [
        Tool(
            name="Crawl Google 4 linkedin profile page", # 에이전트가 이 도구 지칭하는 용어
            func=get_profile_url_tavily, # 이 도구가 실행하기를 원하는 함수
            description="useful for when you need get the Linkedin Page URL", # LLM이 이 도구 사용할지 말지 정하는 부분으로 중요
        )
    ]
